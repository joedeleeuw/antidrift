...
const codebaseAtlasRoot = process.env.CODEBASE_ATLAS_REPO ?? "/Users/sushi/code/codebase-atlas";
...
describe("schemaRoundtripInventory", () => {
  it("classifies real Codebase Atlas schema roundtrips without blocking", () => {
    ...
    const moveTo = app?.findings.find((finding) => finding.enclosing === "LanternController.moveTo");
    ...
    const markUnderstood = app?.findings.find((finding) => finding.enclosing === "markExplorationTileUnderstood");
