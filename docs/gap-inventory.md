...
| State                      | Count | Meaning                                                                                |
| -------------------------- | ----: | -------------------------------------------------------------------------------------- |
| Stable                     |     4 | Ready, independently replicated enough for stable treatment.                           |
...
| ---: | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
|    1 | `antidrift/no-unsafe-deserialize` nested parsed-subfield drift                  | Find or unblock one real program where code parses a broad JSON value, then parses/uses a nested subfield as trusted data.                   | Parse-at-edge is original scope, the rule is already useful, and this is the clearest remaining semantic gap.                              |
|    2 | `antidrift/no-sql-string-concat` sanitized identifier replication               | Find one more real sanitized dynamic identifier clean control and one near-miss guard that still reports.                                    | Security value is high and the rule is already narrowed; promotion is blocked by evidence, not implementation architecture.                |
|    3 | `antidrift/no-cast-to-branded` adoption or retirement                           | Decide whether real consumers will adopt `@joedeleeuw/antidrift/brand`; otherwise retire to ecosystem-covered.                               | Brands are core to "cannot cast your way to authority," but no real forged-brand drift exists yet. This needs a decision before more code. |
|    4 | `antidrift/no-defensive-shape-probing` second drift repo                        | Search for another broad `any`/`unknown` mini-parser that probes many possible fields instead of validating through an owner.                | This maps directly to the "getBoolean checks every property" concern, but must stay narrow to avoid flagging ordinary predicates.          |
|    5 | `antidrift/no-underchecked-type-predicate` second drift and delegation pressure | Find another broad-input object predicate that claims a contract without checking asserted fields, plus a delegated-validator clean control. | This is the stronger, type-aware answer to unsafe predicate authority. It is high value, but needs more real drift.                        |
|    6 | `antidrift/no-canonical-model-fork` second configured repo                      | Run a configured domain-owner inventory in another repo with real first-party model owners.                                                  | One-owner-per-concept is the project thesis; the rule is registry-gated enough to avoid broad heuristic matching.                          |
|    7 | `antidrift/no-structural-type-fork` independent replication                     | Seek independent installed-package or generated-source fork replication and more boundary DTO clean pressure.                                | Core rule, already mature locally, but stable promotion still needs independent replication outside current anchors.                       |
|    8 | `antidrift/no-coupled-state-setters` inventory classification                   | Classify broad Chaski findings into true state-machine drift, acceptable handlers, and false positives.                                      | React state shape is original scope, but broad findings can become noisy if not classified before tightening.                              |
|    9 | `antidrift/no-status-triplet-state` low-noise inventory or retirement           | Run another frontend inventory with configured name groups, then keep off or retire if signal stays too heuristic.                           | The smell is real, but the signal is name-group-based by nature. Treat as evidence-gathering, not implementation.                          |
|   10 | `antidrift/no-nullable-positional-tuple` second tuple program                   | Find another real nullable/optional multi-slot tuple and clean tuple controls.                                                               | Deterministic AST and low false-positive risk, but narrower impact than parse/cast/model drift.                                            |
|   11 | `antidrift/no-inline-structural-type-at-use-site` replication and exclusions    | Seek independent boundary-function drift plus UI props/callback clean pressure.                                                              | Valuable type-contract rule, but likely expressible by config in some shapes and needs more real pressure.                                 |
|   12 | `antidrift/require-authz-check` scope decision                                  | Decide whether middleware dominance or tRPC procedure authorization is in scope before writing code.                                         | Security value is high, but wrong widening could create a large, framework-specific rule. Scope first.                                     |
|   13 | `antidrift/no-raw-tailwind-color` inventory classification                      | Classify broad design-system findings before stable claims.                                                                                  | Useful policy, but less central to the original type/parse/authority problem and likely inventory-heavy.                                   |
|   14 | `antidrift/no-hover-translate-card` second UI inventory and CSS boundary        | Run another UI repo inventory; decide whether CSS `@apply` is in or out.                                                                     | Small, deterministic, and bounded, but lower strategic leverage.                                                                           |
|   15 | `antidrift/no-status-literal-in-type` independent status-fork replication       | Find another configured domain-status redeclaration outside the owner.                                                                       | Useful registry-backed drift check, but status evidence is less urgent than model/type forks.                                              |
|   16 | `antidrift/no-role-literal-in-type` real role redeclaration or retire           | Find a real multi-literal role-union redeclaration, or keep default-off/retire.                                                              | Role values are generic and false-positive-prone without strong type context.                                                              |
|   17 | `antidrift/no-async-array-method` second drift or exception                     | Find another independent async array-method drift instance or explicitly accept one-repo evidence.                                           | Deterministic and useful, but lower leverage and already partly adjacent to ecosystem rules.                                               |
|   18 | `antidrift/no-unsafe-cast-chain` independent replication                        | Seek another double-cast tunnel, otherwise keep behind `no-appeasement-cast`.                                                                | Narrow tunnel is mostly subsumed by the broader appeasement-cast rule.                                                                     |
|   19 | `antidrift/no-obvious-comment` inventory or retire                              | Run another inventory only if comment-quality enforcement becomes important.                                                                 | Token-overlap heuristic; not central enough to spend custom-rule time without strong evidence.                                             |
...
| Rank | Policy gap                                                           | Why                                                                                                                                         |
| ---: | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
|    1 | `errors/preserve-caught-error` and `errors/no-fallback-to-empty`     | Error swallowing/fallback behavior is a real agent failure mode and has a clearer construction pattern than generic silent-catch detection. |
|    2 | `boundary/no-env-access-in-client`                                   | Side-effect boundary rule with likely deterministic file/context checks.                                                                    |
...
|    5 | Secret scanning                                                      | High value, but likely should be delegated to a maintained secret scanner rather than custom AST code.                                      |
|    6 | `auth/no-boundaryless-route` and `auth/no-client-only-authorization` | Valuable but framework-specific; should wait until authz scope is settled.                                                                  |
|    7 | Feature scatter and high-fan-in growth                               | Architecture-health signals, but not likely to be deterministic enough for a blocking local ESLint rule.                                    |
|    8 | Observability async-boundary/fire-and-forget tracking                | Potentially valuable, but needs a concrete local logging/tracing convention first.                                                          |
|    9 | MCP allowlist/version controls                                       | Supply-chain useful, but belongs in policy scripts/config checks, not source-code lint.                                                     |
...
| `antidrift/no-defensive-shape-probing`            | Ready, not stable         | Has clean breadth but only one drift repository. Needs another real broad-value mini-parser drift and broad inventory for the `any`/`unknown` entries signal.                                              |
...
| `antidrift/no-sql-string-concat`                  | Ready, not stable         | Needs a second independent sanitized dynamic SQL identifier clean control and a real invalid/near-miss guard control before stable promotion.                                                              |
...
| `antidrift/no-underchecked-type-predicate`        | Ready, not stable         | Needs another broad-input underchecked predicate drift case. Delegated validator helpers still need provenance pressure.                                                                                   |
...
| `antidrift/no-unsafe-deserialize`                 | Ready, not stable         | Needs an evaluated nested parsed-subfield drift gate, such as parsing a subfield from a broad parsed object. Cloudflare candidates are blocked by unresolved `agents/tsconfig`.                            |
...
| `error-handling`              | Partial          | Preserve-caught-error and fallback-to-empty are not implemented.                                                                                                                                                                          |
...
| --------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Cloudflare example package tsconfigs extend `agents/tsconfig`, which is not install-resolvable in the external clone. | `antidrift/no-unsafe-deserialize`   | Four Cloudflare deserialize cases are recorded as `known-gap`, including the desired nested parsed-subfield drift candidate.                        |
| SQL sanitized dynamic identifier proof exists in only one independent repo.                                           | `antidrift/no-sql-string-concat`    | Keep ready but not stable until another repo supplies the clean shape and a near-miss/invalid guard remains reported.                               |
...
1. `antidrift/no-unsafe-deserialize`: find or unblock a real nested parsed-subfield drift program under a resolvable TypeScript config.
2. `antidrift/no-sql-string-concat`: find a second sanitized dynamic SQL identifier clean control and one invalid/near-miss guard that still reports.
