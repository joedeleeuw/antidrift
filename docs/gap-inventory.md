...
|    3 | `antidrift/no-defensive-shape-probing` second drift repo                        | Search for another broad `any`/`unknown` mini-parser that probes many possible fields instead of validating through an owner.                | This maps directly to the "getBoolean checks every property" concern, but must stay narrow to avoid flagging ordinary predicates.          |
