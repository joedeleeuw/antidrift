...
| ---: | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
|    1 | `antidrift/no-sql-string-concat` sanitized identifier replication               | Find one more real sanitized dynamic identifier clean control and one near-miss guard that still reports.                                    | Security value is high and the rule is already narrowed; promotion is blocked by evidence, not implementation architecture.                |
|    2 | `antidrift/no-defensive-shape-probing` second drift repo                        | Search for another broad `any`/`unknown` mini-parser that probes many possible fields instead of validating through an owner. Prefer `unknown`-typed drift or benchmark evidence not already explained by upstream unsafe rules. | This maps directly to the "getBoolean checks every property" concern, but current `any` drift partially overlaps `@typescript-eslint/no-unsafe-member-access`; keep the custom value to grouped architectural feedback. |
...
1. `antidrift/no-sql-string-concat`: find a second sanitized dynamic SQL identifier clean control and one invalid/near-miss guard that still reports.
