...
- `policy:validate-external-corpus` is the optional fallback real-corpus gate for non-Chaski repos when Chaski has only clean controls for an implemented rule. It currently asserts Sudocode cases from `SUDOCODE_REPO` or `/Users/sushi/code/sudocode-main` and Codebase Atlas cases from `CODEBASE_ATLAS_REPO` or `/Users/sushi/code/codebase-atlas`, then skips if no external repo exists. For promotion/slice-completion breadth, run `antidrift external-corpus --min-repositories 2`.
