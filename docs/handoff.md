...
- `policy:validate-external-corpus` is the optional fallback real-corpus gate for non-Chaski repos when Chaski has only clean controls for an implemented rule. It currently asserts `no-async-array-method`, `no-unsafe-deserialize`, and the Express-style `require-authz-check` against `SUDOCODE_REPO` or `/Users/sushi/code/sudocode-main` and skips otherwise.
