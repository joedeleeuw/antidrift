...
SonarQube is the PR and trend governance layer. Local enforcement remains in Oxlint, the reduced ESLint TypeChecker pass, TypeScript, Vitest, and hooks.
...
`pnpm sonar:prepare` converts the reduced custom typed-rule ESLint JSON into Sonar generic external issue format at:
...
This package does not bundle a Trunk-style multi-language lint runner. Keep Antidrift focused on TypeScript source semantics: Oxlint for native and supported type-aware findings, a reduced ESLint pass for custom TypeChecker rules, Vitest and `tsc` for local correctness, and Sonar for portfolio analysis and gate metrics.
...
- Exclude native Sonar rules already owned by enabled Oxlint, TypeScript, React, Vitest, Unicorn, or Antidrift rules.
...
Use a Sonar Way-derived JavaScript and TypeScript profile as the baseline. Do not treat Sonar as the custom Antidrift rule host: Sonar owns broad portfolio analysis and gate metrics; Oxlint owns local native and supported type-aware feedback; ESLint owns only custom TypeChecker rules. Oxlint's source complexity metrics are not equivalent to Sonar cognitive complexity. `pnpm sonar:prepare` imports the reduced ESLint findings as generic external issues so custom typed diagnostics appear beside native Sonar findings.
...
| Complexity and duplication     | `S3776`, `S2004`, `S107`, `S1479`, `S4624`, `S4144`, `S3516` plus duplication metrics in the gate                                                                                                                                                                                               | Sonar owns cognitive complexity, interface sprawl, copy-paste growth, and trend/gate posture. Oxlint catches cyclomatic complexity, depth, and parameter count locally.                           |
...
| External antidrift             | Imported `antidrift/*` findings                                                                                                                                                                                                                                                                 | Sonar reports these, but the TypeChecker rules remain managed in the reduced ESLint pass because external rules are not part of Sonar quality profiles.                                           |
...
Do not add style/import formatting to the Sonar profile. Oxlint owns configured import and type-import correctness; Prettier owns formatting and sorting conventions.
