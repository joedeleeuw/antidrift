...
- `sonarjs/sql-queries` is active through `sonarjs.configs.recommended`; `pnpm policy:benchmark-sql-queries` checked 262 real files and reported 16 `antidrift/no-sql-string-concat` findings versus 0 `sonarjs/sql-queries` findings. Keep the custom SQL interpolation guard.
