...
- `sonarjs/sql-queries` is active through `sonarjs.configs.recommended`; `pnpm policy:benchmark-sql-queries` checked 268 real files and reported 16 `antidrift/no-sql-string-concat` findings versus 0 `sonarjs/sql-queries` findings. Keep the custom SQL interpolation guard.
...
| `antidrift/no-sql-string-concat`             | Keep custom    | `sonarjs/sql-queries`, `sql/no-unsafe-query`, or SQL-template plugins                            | The measured SonarJS benchmark reported 0 findings against 16 custom findings after narrowing placeholder lists, numbered Postgres placeholder fragments, static SQL fragment arrays, SQL tags, and identifier proofs. SQL plugins are oriented around chosen SQL/tagged-template conventions; keep custom unless a supported query convention replaces this HogQL/template interpolation delta. |
