...
| `antidrift/no-cast-to-branded`               | Hold custom    | `@typescript-eslint/no-unsafe-type-assertion`                                                    | No real forged brand cast exists yet. Keep default-off only for brand-specific guidance and retire to ecosystem-covered if a second independent repo still produces no real non-test forged brand cast by the next promotion cycle.                                                                                |
...
| `antidrift/no-role-literal-in-type`   | Keep custom, under-proven | Generated `no-restricted-syntax` selectors from the domain registry                                    | Same owner-file/context reason as status literals, but the role values are more generic. Keep default-off until a multi-literal role-union signal and real role-fork evidence exist.          |
