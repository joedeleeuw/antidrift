...
- `sonarjs/sql-queries` is active through `sonarjs.configs.recommended`; `pnpm policy:benchmark-sql-queries` checked 190 real files and reported 10 `antidrift/no-sql-string-concat` findings versus 0 `sonarjs/sql-queries` findings. Keep the custom SQL interpolation guard.
...
| `antidrift/no-sql-string-concat`             | Keep custom    | `sonarjs/sql-queries`, `sql/no-unsafe-query`, or SQL-template plugins                            | The measured SonarJS benchmark reported 0 findings against 10 custom findings in real Chaski SQL-like programs after narrowing placeholder lists and static SQL fragment arrays. SQL plugins are oriented around chosen SQL/tagged-template conventions, so keep custom until the repo chooses a query convention. |
