...
- **Mining + adversarial verification** — ultracode workflow `change-contract-mvp-evidence-gauntlet`
  ...
  text *before* reading the diff; every candidate adversarially verified by 3 default-refute skeptics;
...
**GO — build the deterministic inventory core (v0, inventory-only).** Real historical commits exist
where a frozen contract deterministically catches undeclared dependency / path / repo-surface drift.
The MVP kill-condition (≥2 unambiguous TPs across ≥2 repos) is met by the 5 gold TPs.
...
| opencode @ `1772e8ee` | LimitsGraph `req` 1400→3200 is runtime content data for the *same* Go pricing surface the docs subject names; rests on reviewer judgment without a pre-frozen docs-only contract |
...
+ regenerated bindings) · `chaski@59e1bee78` (declared mock ERP removal) · `chaski@9da69cc59`
(declared docs diagrams). The detector did not over-fire on any.

## Caveats carried forward (block enforcement, not v0 inventory)

1. **Zero freshly-mined clean controls** → detector false-positive *rate* is unquantified.
...
1. **Build v0 inventory-only spine now**, seeded with the 5 gold TPs + 7 TN controls above as the real
   validation corpus. Quarantine the opencode cases as ambiguous inventory, never gold.
