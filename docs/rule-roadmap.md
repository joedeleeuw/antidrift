...
| Spec-only: `arch/max-function-lines`, `arch/max-component-lines`, `ui/no-generic-ai-copy`, `obs/async-boundary-requires-context`, `sec/no-hardcoded-secret` | Various                                                                                                                                    | Raw line-count/component-size and non-TS scanner surfaces remain owned by Sonar trends, maintained scanners, future policy scripts, or review guidance. Local cyclomatic complexity, depth, and params are covered by `antidrift oxlint`, not custom AST rules. Agent-ops config/runtime/doc candidates are tracked in the Agent-Ops Roadmap above.                                                                                                |
