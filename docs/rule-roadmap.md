...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A. Local AST/source-shape proof**            | One file's syntax, import bindings, string literals, or lexical scope proves the construction                                                                                  | Oxlint native rule or Antidrift JavaScript plugin rule                                                                                                                          | `antidrift/require-effect-deps`, never-await branch of `antidrift/no-async-array-method`, `antidrift/no-inline-structural-type-at-use-site`, `antidrift/no-raw-fetch-in-component`, JSX class-string design rules                                                                                                                                                                                                    | Can block when the construction is the violation. If source shape only names a suspect, demote to inventory or move to B/C/F.                                                                             |
| **B. Semantic source/type/provenance proof**   | AST plus local dataflow, setter transition evidence, TypeChecker facts, schema provenance, SQL/context control flow, or validated broad-input flow proves the contract failure | Oxlint JavaScript plugin for local proof; reduced ESLint pass only when TypeChecker data is required                                                                            | `antidrift/no-contract-appeasement-projection`, `antidrift/no-appeasement-cast`, `antidrift/no-unsafe-deserialize`, `antidrift/no-sql-string-concat`, `antidrift/no-underchecked-type-predicate`, `antidrift/no-redundant-zod-parse`, `antidrift/react-max-component-props`, blocking branches of `antidrift/no-handrolled-resource-lifecycle-cells`, opt-in map/flatMap branch of `antidrift/no-async-array-method` | Can block only after the semantic proof and fallback behavior are characterized against drift and clean controls.                                                                                         |
| **C. Authority-index ownership proof**         | Source shape plus accepted package, domain, generated, authz, or owner facts proves the code forked or bypassed an owner                                                       | Registry-backed Oxlint or TypeChecker rule, or generated Oxlint config                                                                                                          | `antidrift/no-structural-type-fork`, `antidrift/no-canonical-model-fork`, `antidrift/no-status-literal-in-type`, `antidrift/require-authz-check`, generated source-owner routing                                                                                                                                                                                                                                     | Registry absence means no blocking proof. These rules must fail closed to "not configured" or advisory, not infer owners from names alone.                                                                |
...
| High-touch file growth and feature scatter | `diff+task+graph` plus git history       | PR policy report                                                                                                                                            | file churn over recent commits, cross-feature touched paths, duplicated feature directories, changed-lines concentration                                           | Use as reviewer triage and Sonar complement; block only protected ownership violations with registry facts                                                                | Do not confuse the 1,500-line module ceiling with history-aware ownership proof        |
...
Stable rules are default-on in the distributable config. A `ready` rule can also be default-on when the registry records an explicit consumer-facing decision; otherwise useful `ready` rules stay exported as default-off inventory until their own evidence clears independent real-repo drift, clean controls, zero known false positives, zero known false negatives in the inspected corpus, no unresolved production concerns, and advisory review.

### `antidrift/react-max-component-props`

- Value: a wide React props contract creates too many component API combinations and usually means the component is carrying several responsibilities through one boundary.
- Problem kind: component API budget, proven by JSX-return component shape plus TypeChecker-resolved locally-owned prop symbols.
- Pattern: split the component, group cohesive props into an owned options/value object, or move data/state responsibility behind a resource/container boundary.
- Ecosystem: React rules cover props per line, unused props, and positional parameter count; exact max-props support exists for Vue, not React.
- Signal: TypeChecker plus JSX-return proof. Inherited DOM/React props and whole-props pass-through wrappers stay clean.
- Evidence: Chaski `MobileFlightTrackerMap` reports 31 locally-owned accepted props; regression fixtures and ecosystem-gap search cover the first implementation.
- False-positive concern: design-system, generated, schema/form, and API-preserving wrapper components need threshold review before stable promotion.
- Status: ready and enabled in `createConfig()` with max 12; stable false until threshold inventory is done.
...
- Value: an internal helper that projects a source value contract into another explicit return contract adds indirection plus a hand-maintained boundary TypeScript cannot prove is owned.
- Problem kind: contract appeasement, proven by TypeChecker source/return ownership plus structural return-flow. The old selector-wrapper branch is one subset; cross-contract discriminant projection is the widened branch.
- Pattern: use the value directly, construct or validate the target contract at its owner, or keep a real exported adapter boundary.
- Ecosystem: none (net-antidrift).
- Signal: TypeChecker plus structural return-flow proof.
- Evidence: selector-wrapper drift in Chaski, Codebase Atlas (`fullExcerpt`), and Murderbox (annotated key-extractor callback); fixture-backed cross-contract projection for `EventKind -> MemoryResult["action"]`; transforming helpers and real remappings stay clean.
- False-positive concern: exported/public boundaries and parser/validator/constructor-backed conversions must stay clean. False-negative concern: local variable indirection and unrelated side-effect calls currently evade the v1 proof.
...
- Value: SQL assembled by interpolation or concatenation reaches the database without a binding or escaping boundary — injection and quoting bugs. No configured SQL plugin currently replaces the custom signal (145 findings on 370 benchmark files; 486 findings on the 24-repo fleet after removing name-only member proof).
...
- Ecosystem: SQL-template plugins assume chosen tag conventions and remain only partial overlap.
...
| Policy rule                                                                                       | Owner                                                                                                                |
| ------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `react/no-derived-state-effect`                                                                   | Oxlint `react/react-compiler` through `EffectDerivationsOfState`                                                     |
| `arch/no-cross-layer-import`                                                                      | `boundaries/element-types` (ecosystem)                                                                               |
| `arch/no-deep-import`                                                                             | `boundaries/no-private` (ecosystem)                                                                                  |
| `arch/no-new-dependency-cycle`                                                                    | Oxlint `import/no-cycle` (custom `no-cycle` locked retired)                                                          |
| `errors/preserve-caught-error`                                                                    | Oxlint `preserve-caught-error`                                                                                       |
| `test/no-only-or-skip`                                                                            | Oxlint native Vitest rules                                                                                           |
| `test/no-conditional-expect`                                                                      | Oxlint `vitest/no-conditional-expect`                                                                                |
| `test/no-test-without-assertion`                                                                  | `vitest/expect-expect` (ecosystem)                                                                                   |
| `policy/no-inline-disable-without-ticket`                                                         | Oxlint-hosted `eslint-comments/require-description` plus native `typescript/ban-ts-comment` (locked)                 |
| `perf/no-await-in-loop-with-io`                                                                   | Oxlint `no-await-in-loop` (IO narrowing only if real noise appears)                                                  |
...
| `agent/block-generated-policy-edits`, `agent/lint-after-edit`, `agent/require-checks-before-stop` | Generated lifecycle hooks                                                                                            |
...
| `sonar/no-new-critical-issues`, `sonar/complexity-budget`                                         | SonarQube server gates plus the local `antidrift oxlint` complexity/depth/params gate                                |
| `arch/max-module-lines`                                                                           | Oxlint `max-lines` with a 1,500-line ceiling that counts blank and comment lines                                     |
| `ts/no-mechanical-get-x-from-y`                                                                   | Merged into `ts/no-contract-appeasement-projection` — the name was the symptom, the projection structure is the rule |

SQL-template plugin coverage is still adjacent rather than a replacement for the custom HogQL/template interpolation signal.
...
| Rule                                     | Why it died                                                                                                                                       |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `no-cycle`                               | Oxlint owns import graphs (`import/no-cycle`).                                                                                                    |
...
| Mined 2026-06-10: no `mocks/` imports in production code                                                                                                    | Test doubles leaking into prod bundles/behavior                                                                                            | Config-covered: generated Oxlint `no-restricted-imports`; needs one real prod mock-import drift before wiring. No custom code.                                                                                                                                                                                                                                                                                                                     |
...
| Spec-only: `arch/max-function-lines`, `arch/max-component-lines`, `ui/no-generic-ai-copy`, `obs/async-boundary-requires-context`, `sec/no-hardcoded-secret` | Various                                                                                                                                    | Function/component-specific size and non-TS scanner surfaces remain owned by Sonar trends, maintained scanners, future policy scripts, or review guidance. Oxlint owns the broad 1,500-line module ceiling plus local cyclomatic complexity, depth, and params, not custom AST rules. Agent-ops config/runtime/doc candidates are tracked in the Agent-Ops Roadmap above.                                                                          |
