...
| Mined 2026-06-10 (frustration pass): diff-scope creep guard | Agents change far more than the task warrants ("why are you changing so much fucking code dude"; "dude the diff is like 200 lines"; "why do we need", 15+ variations — many recurrence across Codex, Claude, and opencode) | Not ESLint: agent-ops hook/policy-script research (changed-lines budget vs task scope); the ecosystem owner for the PR-time variant is Danger-style size checks in CI. The exhaustive sweep confirmed it at n=60, heading an agent-behavior taxonomy of ~225 items (skip-verification, scope-drift, instruction-skipping, over-automation — see `reports/complaint-sweep-2026-06-10.json`); the hook layer is the package's growth surface. |
| Spec-only: `arch/max-function-lines`, `arch/max-component-lines`, `arch/no-high-touch-file-growth`, `arch/no-feature-scatter`, `ui/no-generic-ai-copy`, `obs/async-boundary-requires-context`, `sec/no-hardcoded-secret`, `mcp/no-unapproved-server`, `mcp/pin-server-version`, `rules/no-vague-agent-rules` | Various | Owned by Sonar trends, maintained scanners, future policy scripts, or review guidance — not custom AST rules. |
