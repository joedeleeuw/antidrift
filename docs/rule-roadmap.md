...
- **Lifecycle hooks first.** Current Claude Code hooks fire per session, per turn, and around tool calls; `PreToolUse` can block risky actions, `PostToolUse` can run changed-file checks, and `Stop` can require final verification evidence. OpenAI's Agents guidance maps the same idea to input/output/tool guardrails plus human review for side-effecting actions.
