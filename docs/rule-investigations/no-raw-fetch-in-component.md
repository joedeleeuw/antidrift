...
- Broad inventory also found Murderbox component-module fetches in `apps/client/app/(models)/admin.tsx`, `apps/client/app/_debug.tsx`, `apps/client/app/(chat)/index.tsx`, and `apps/client/src/components/chat/message-list.tsx`.
...
- Murderbox client: 192 checked files, 6 findings.
...
Independent drift now exists. Stable promotion should wait for the explicit scope decision: keep the rule as a narrow `fetch`-specific component rule, or rename/extend it into a registry-backed raw transport boundary rule.
