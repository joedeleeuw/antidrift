...
- The widened SQL scan is now reproducible, but still not fully classified. The targeted broad inventory already proves known non-type-aware conservative reports on escaped identifier controls, so stable promotion still requires broad finding classification.
...
Stable promotion waits on broad finding classification. Safe identifier controls remain parser-services-only; do not add name-only exemptions to make non-type-aware inventory quiet.
