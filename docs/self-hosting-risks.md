...
| 6    | Hook creep                | Hooks start duplicating semantic lint rules, creating two enforcement paths and inconsistent failure messages.       | Hooks are limited to generated-file protection, changed-file checks, and stop/session verification.                                                                         | Put code semantics in ESLint unless the check needs repo-local agent lifecycle context.                                              |
| 7    | Retired-engine creep      | A second lint engine returns through scripts, exports, or generated artifacts as a broad semantic lint path.         | ESLint is the semantic lint path; `antidrift oxlint` is allowed only as the packaged complexity/depth/params gate, and `docs/lint-rule-parity.md` is the historical ledger. | Treat broader engine usage as `[policy-change]` work with a rule-by-rule migration plan.                                            |
