...
| 2    | Rule surface drift        | A custom rule is exported but not configured, assigned to two runtimes, or lacks corpus evidence.                    | `pnpm policy:check-rule-surface` compares plugin exports, Oxlint and ESLint ownership, severity, and corpus coverage.        | Keep behavioral tests close to the runtime that executes each rule.                                                                 |
...
| 6    | Hook creep                | Hooks start duplicating semantic lint rules, creating two enforcement paths and inconsistent failure messages.       | Hooks are limited to generated-file protection, changed-file checks, and stop/session verification.                          | Put code semantics in the owned lint runtime unless the check needs repo-local agent lifecycle context.                             |
| 7    | Dual-runtime duplication  | Equivalent Oxlint and ESLint rules run together, adding maintenance and inconsistent failure messages.               | `policy:check-rule-surface` rejects custom rules enabled by both runtimes, and `docs/lint-rule-parity.md` records ownership. | Move ownership atomically: prove parity, enable the new owner, and delete the old owner in the same change.                         |

## Hook vs Rule vs Doc

Use **Oxlint rules** for native, type-aware ecosystem, AST, scope, import, and control-flow checks. Use **ESLint rules** only for custom TypeScript checker semantics unavailable to Oxlint JavaScript plugins.
...
Use **hooks** when the check depends on repo-local agent tool lifecycle. Examples: blocking edits to generated instruction files and forcing session verification before stop. Workstation shell safety belongs to the user's agent-control layer.
