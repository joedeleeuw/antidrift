...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | AST string construction                                   | `partial-overlap` / `own-antidrift`    | Sanitized dynamic SQL identifier proof is covered by Cloudflare Workspace; run the formal advisory review before stable promotion.                              |
