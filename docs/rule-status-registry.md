...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | SQL keyword/context AST plus scope-binding guard control-flow | `partial-overlap` / `own-antidrift`    | Run a fresh stable-promotion review now that lower-edge and imported-escaper proof are covered by real corpus gates.                                          |
