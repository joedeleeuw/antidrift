...
| `antidrift/no-defensive-shape-probing`            |        `ready` |     no | TypeChecker plus AST shape                                    | `partial-overlap` / `own-antidrift`    | Current any-typed drift overlaps upstream unsafe rules on member reads; seek unknown-typed drift or a second repo before stable promotion.                      |
...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | SQL keyword/context AST plus scope-binding guard control-flow | `partial-overlap` / `own-antidrift`    | Seek a second independent sanitized dynamic SQL identifier clean control and a real invalid/near-miss guard control before stable promotion.                    |
...
| `antidrift/no-unsafe-deserialize`                 |        `ready` |     no | TypeChecker plus local string-boundary control flow           | `partial-overlap` / `own-antidrift`    | Second evaluated drift repo is now opencode; add an evaluated nested parsed-subfield gate before stable promotion.                                              |
