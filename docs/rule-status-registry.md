...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | AST string construction                                   | `partial-overlap` / `own-antidrift`    | SonarJS benchmark found 0 upstream findings against 10 custom findings; static fragments and SQL tags are clean, identifier fragments remain unresolved. |
