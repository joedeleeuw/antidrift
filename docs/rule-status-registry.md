...
| `antidrift/no-raw-fetch-in-component`             |        `ready` |     no | import-scope plus AST                                     | `config-replacement` / `own-antidrift` | Run final grounded advisory and promotion review; keep broader raw transport as a separate future rule until real non-fetch drift appears.    |
...
| `antidrift/no-redundant-zod-parse`                |        `ready` |     no | TypeChecker plus schema provenance                        | `net-antidrift` / `own-antidrift`      | Decide whether to fix or explicitly accept inline awaited service-result reparses and sync helper-result reparses before stable promotion.    |
...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | AST string construction                                   | `partial-overlap` / `own-antidrift`    | SonarJS benchmark found 0 upstream findings against 11 custom findings; adjudicate dynamic SQL clause fragments before stable promotion.      |
...
| `antidrift/no-unsafe-deserialize`                 |        `ready` |    yes | TypeChecker plus local string-boundary control flow       | `partial-overlap` / `own-antidrift`    | Stable; monitor raw-plugin type-service guard feedback and real parse-at-edge regressions.                                                   |
...
| `ecosystem/sonar-sql-queries`                  | `ecosystem-covered` | maintained ESLint plugin         | Covered by `sonarjs/sql-queries`, but the local benchmark currently reports 0 findings against 11 custom SQL findings.                             |
