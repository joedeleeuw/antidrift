...
| `antidrift/no-unsafe-deserialize`                 |        `ready` |     no | TypeChecker                                               | `partial-overlap` / `own-antidrift`    | Find a second independent broad-input JSON.parse drift instance, or document an explicit one-repo evidence exception before stable promotion. |
