...
| `antidrift/no-redundant-zod-parse`                |        `ready` |     no | TypeChecker plus schema provenance                        | `net-antidrift` / `own-antidrift`      | Seek a real negative gate for consumed assertion-callback reparses, or explicitly accept that edge before stable promotion.                   |
