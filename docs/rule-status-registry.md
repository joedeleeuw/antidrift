...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | SQL keyword/context AST plus scope-binding guard control-flow plus TypeChecker imported-escaper proof | `partial-overlap` / `own-antidrift`    | Classify broad SQL findings and resolve or document non-type-aware imported-escaper degradation before stable promotion.                                      |
