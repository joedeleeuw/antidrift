...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | AST string construction                                   | `partial-overlap` / `own-antidrift`    | Keep ready but not stable until sanitized dynamic SQL identifier controls either stay clean through a narrow proof or are explicitly ruled out of scope.        |
...
| `antidrift/no-unsafe-deserialize`                 |        `ready` |     no | TypeChecker plus local string-boundary control flow       | `partial-overlap` / `own-antidrift`    | Re-establish a second evaluated drift repository and add an evaluated nested parsed-subfield gate before stable promotion.                                      |
