...
- `antidrift/no-defensive-shape-probing`: broad-value mini-parser inventory signal; needs a second drift source that is not already explained by upstream unsafe-member rules before blocking.
...
| `antidrift/no-defensive-shape-probing`            | `under-proven` |     no | TypeChecker plus AST shape                                                                            | `partial-overlap` / `own-antidrift`    | Inventory/off unless a second drift proves value beyond upstream unsafe-member checks.                                                                                             |
...
| `antidrift/no-status-triplet-state`               | `under-proven` |     no | React `useState` name groups for resource-lifecycle inventory                                         | `partial-overlap` / `own-antidrift`    | Decide fold-in to `no-coupled-state-setters` with a redundant-constant-cell branch, or retire the standalone rule.                                                                |
| `antidrift/no-structural-type-fork`               |        `ready` |     no | TypeChecker structural comparison plus generated registry                                             | `net-antidrift` / `own-antidrift`      | Decide installed-package owner authority and projected DTO boundaries; generated/registry modes are the stronger blocking path.                                                     |
