...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | SQL keyword/context AST plus scope-binding guard control-flow plus TypeChecker imported-escaper proof | `partial-overlap` / `own-antidrift`    | Add type-aware broad SQL plans for PowerSync modules, classify broad SQL findings, and resolve or explicitly accept non-type-aware imported-escaper / escaped-identifier behavior. |
