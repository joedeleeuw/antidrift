...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | SQL keyword/context AST plus scope-binding guard control-flow | `partial-overlap` / `own-antidrift`    | Seek a real invalid/near-miss guard or escaper control before stable promotion.                                                                                 |
