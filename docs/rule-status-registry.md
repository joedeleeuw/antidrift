...
| `antidrift/no-unsafe-deserialize`                 |        `ready` |     no | TypeChecker plus local string-boundary control flow       | `partial-overlap` / `own-antidrift`    | Close the type-aware parserServices production guard/documentation slice before stable promotion.                                             |
