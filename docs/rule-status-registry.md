...
- `antidrift/no-mocked-response-body-oracle`: mocked `response.json()` object-shape assertions remain default-off inventory until independent repositories pressure same-test mock conjunctions and effect-preserving clean controls.
...
| `antidrift/no-mocked-response-body-oracle`         |                `ready` |     no | deterministic AST plus same-test mock arrangement and local `response.json()` provenance                                                                     | `net-antidrift` / `own-antidrift`      | Default-off until independent corpora confirm the deletion signal and effect-preserving clean boundary.                                                                      |
