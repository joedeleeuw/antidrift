...
| `antidrift/no-sql-string-concat`                  |        `ready` |    yes | AST string construction                                   | `partial-overlap` / `own-antidrift`    | Stable; SonarJS benchmark found 0 upstream findings against 10 custom findings, drift replicates across Chaski/Sudocode/Cloudflare, and clean controls cover static fragments, SQL tags, typed ORDER BY, static column maps, and serialized payload text. |
...
| `antidrift/no-thin-typed-factory-wrapper`      |          `research` | TypeChecker symbol-identity exact-forward delegation | Broad typed-constructor anchors are classified clean; keep only exact-forward research and drop unless exact-forward drift appears in two independent repos. |
