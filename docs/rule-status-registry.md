...
|---|---:|---:|---|---|---|
| `antidrift/no-appeasement-cast` | `ready` | no | TypeChecker | `broader-upstream` / `own-antidrift` | Seek an independent second-repo replication before stable promotion. |
| `antidrift/no-async-array-method` | `ready` | no | AST | `partial-overlap` / `own-antidrift` | Seek inventory results without false positives before stable promotion. |
| `antidrift/no-cast-to-branded` | `under-proven` | no | TypeChecker | `broader-upstream` / `own-antidrift` | Wait for a real branded boundary before promotion work. |
...
| `antidrift/no-defensive-shape-probing` | `under-proven` | no | deterministic AST | `net-antidrift` / `own-antidrift` | Decide whether the construction pattern itself should be banned; otherwise add TypeChecker or registry evidence before local readiness. |
...
| `antidrift/no-raw-fetch-in-component` | `ready` | no | import-scope plus AST | `config-replacement` / `own-antidrift` | Seek independent frontend replication before stable promotion. |
...
| `antidrift/no-redundant-zod-parse` | `ready` | no | TypeChecker plus schema provenance | `net-antidrift` / `own-antidrift` | Seek independent service-to-router reparse replication before stable promotion. |
...
| `antidrift/no-silent-catch` | `ready` | no | AST | `partial-overlap` / `own-antidrift` | Seek independent replication before stable promotion. |
| `antidrift/no-sql-string-concat` | `ready` | no | AST string construction | `partial-overlap` / `own-antidrift` | Seek independent replication with parameterized-query clean controls. |
...
| `antidrift/no-unsafe-deserialize` | `ready` | no | TypeChecker | `partial-overlap` / `own-antidrift` | Seek another repo with both any/unknown JSON.parse drift and typed-string clean parses. |
