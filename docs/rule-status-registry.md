...
| `antidrift/no-trivial-selector-wrapper`           |        `ready` |     no | AST structural return shape                               | `net-antidrift` / `own-antidrift`      | One strong adapter/key-extractor pressure case is documented; keep local reasoned disables until repeated evidence justifies an exemption. |
