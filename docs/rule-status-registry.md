...
| ------------------- | ----: | -------------------------------------------------------------------------------------------- |
| `active-custom`     |    17 | Implemented by an active `antidrift/*` rule, including default-off under-proven inventory rules. |
...
| `antidrift/no-raw-fetch-in-component`             |        `ready` |     no | JSX-return lexical frame plus AST                                                                                                           | `config-replacement` / `own-antidrift` | Re-inventory after removing file-extension, PascalCase-name, and JSX-bearing-module helper heuristics.                                                                    |
...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | SQL keyword/context AST plus scope-binding guard control-flow plus TypeChecker imported-escaper and configured safe-identifier-member proof | `partial-overlap` / `own-antidrift`    | Non-blocking until SQL-builder tag/member APIs are proven by symbol/type provenance instead of name trust or parked known-gap cases.                                      |
...
| `antidrift/no-structural-type-fork`               |        `ready` |     no | TypeChecker exact structural comparison plus authority-index facts                                                                          | `net-antidrift` / `own-antidrift`      | Keep exact generated and accepted package owner copies blocking; projected subsets stay clean.                                                                            |
