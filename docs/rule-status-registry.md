...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | SQL keyword/context AST plus scope-binding guard control-flow plus TypeChecker imported-escaper proof | `partial-overlap` / `own-antidrift`    | Classify broad SQL findings and measured non-type-aware imported-escaper behavior before stable promotion; keep SQL tag, guard-shape, and builder-append inventory running. |
