...
| ------------------- | ----: | -------------------------------------------------------------------------------------------- |
| `active-custom`     |    17 | Implemented by an active `antidrift/*` rule.                                                 |
...
| `antidrift/no-async-array-method`                 |        `ready` |     no | AST                                                                                                                                         | `partial-overlap` / `own-antidrift`    | Decide branch-specific promotion: never-await methods are discarded-return hazards; `map`/`flatMap` not-collected has a separate dataflow burden.                         |
| `antidrift/no-handrolled-resource-lifecycle-cells`              |        `ready` |     no | source binding plus local transition/control-flow proof                                                                                     | `net-antidrift` / `own-antidrift`      | Deterministic lifecycle proof is the blocking core; seven-repo broad co-mutation inventory has 0 blocking lifecycle diagnostics, so promotion needs real lifecycle drift. |
...
| `antidrift/no-raw-tailwind-color`                 |        `ready` |     no | class-string                                                                                                                                | `config-replacement` / `own-antidrift` | Decide lint-vs-theme control layer; if lint stays, classify inventory findings and make token coverage explicit.                                                          |
...
| `antidrift/no-sql-string-concat`                  |        `ready` |    yes | SQL keyword/context AST plus scope-binding guard control-flow plus TypeChecker imported-escaper and configured safe-identifier-member proof | `partial-overlap` / `own-antidrift`    | Stable; keep SQL benchmarks as regression gates before widening tag, guard, placeholder arithmetic, or builder-append coverage.                                           |
| `antidrift/no-status-literal-in-type`             |        `ready` |     no | authority-index plus AST context                                                                                                            | `config-replacement` / `own-antidrift` | Seek independent status-fork replication before stable promotion.                                                                                                         |
| `antidrift/no-structural-type-fork`               |        `ready` |     no | TypeChecker structural comparison plus authority-index facts                                                                                | `net-antidrift` / `own-antidrift`      | Keep generated and accepted package owners blocking; use unconfigured installed-package matches as analytics that proposes accepted package-owner facts.                  |
| `antidrift/no-canonical-model-fork`               |        `ready` |     no | TypeChecker plus domain registry                                                                                                            | `net-antidrift` / `own-antidrift`      | Run adversarial review and boundary DTO/view-model clean-pressure classification before stable promotion.                                                                 |
...
| `antidrift/require-authz-check`                   |        `ready` |     no | AST control-flow plus registry                                                                                                              | `net-antidrift` / `own-antidrift`      | Decide whether typed policy-wrapper route registration should replace absence-of-call detection before widening framework scope.                                          |
...
| `antidrift/no-status-triplet-state`                | Retired after the behavior-based resource-lifecycle proof moved into `no-handrolled-resource-lifecycle-cells`; standalone configurable name groups are inventory, not enforcement.                               |
