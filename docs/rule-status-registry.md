...
| `antidrift/no-handrolled-resource-lifecycle-cells`              | `under-proven` |     no | source binding plus local transition/control-flow proof                                                                                     | `net-antidrift` / `own-antidrift`      | Fixed lifecycle proof finds 2 Chaski diagnostics while broad co-mutation stays inventory-only; validate another repo and review positives before enabling. |
