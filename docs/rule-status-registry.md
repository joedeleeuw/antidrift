...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | AST string construction plus scope-binding guard control-flow | `partial-overlap` / `own-antidrift`    | Seek a second independent sanitized dynamic SQL identifier clean control and a real invalid/near-miss guard control before stable promotion.                    |
