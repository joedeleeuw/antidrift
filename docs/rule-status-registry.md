...
| `antidrift/no-status-triplet-state`               |        `ready` |     no | React `useState` name groups for resource-lifecycle inventory                                         | `partial-overlap` / `own-antidrift`    | Decide fold-in to `no-coupled-state-setters` with a redundant-constant-cell branch, or retire the standalone rule.                                                                |
