...
| `antidrift/no-sql-string-concat`                  |        `ready` |     no | SQL keyword/context AST plus scope-binding guard control-flow | `partial-overlap` / `own-antidrift`    | Decide whether to implement imported SQL escaper proof or explicitly keep cross-file escapers outside stable promotion scope.                                  |
