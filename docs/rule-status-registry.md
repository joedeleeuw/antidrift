...
| `antidrift/no-trivial-selector-wrapper`           |        `ready` |     no | AST structural return shape                               | `net-antidrift` / `own-antidrift`      | Codebase Atlas supplies independent non-name-gated drift; run advisory classification for adapter/key-extractor false-positive risk.           |
...
| `antidrift/require-effect-deps`                   |        `ready` |     no | import binding plus AST                                   | `partial-overlap` / `own-antidrift`    | External drift exists; define an intentional-every-render escape convention before stable promotion.                                           |
