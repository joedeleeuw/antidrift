...
- **Same-schema state recertification**: keep `no-same-schema-recertification` as research, leaning drop. Codebase Atlas has `markExplorationTileUnderstood` as a possible redundant state roundtrip and `moveTo` as a likely valid invariant checkpoint in the same file; that is not enough for implementation.
