...
|    2 | `antidrift/no-unsafe-deserialize`       | narrowing concern              | continue                           | Sudocode, Cloudflare Agents      | Chaski, Codebase Atlas, Sudocode, Cloudflare Agents | Parse-at-edge is core scope; type-aware clean breadth exists and drift now replicates across two repos; stable is blocked by whether direct `typeof` string guards over `any` should be exempted.                                                                          |
...
The `antidrift/no-unsafe-deserialize` slice now has second-repo drift replication through Cloudflare Agents and still has clean controls in Chaski, Codebase Atlas, Sudocode, and Cloudflare Agents. The next useful rule work is to resolve the guarded-`any` narrowing concern, not to keep searching for more JSON.parse drift.

1. Add or reject an AST/control-flow exemption for `typeof <same expression> === "string"` guards over `any` before `JSON.parse`.
2. Keep Cloudflare `examples/assistant/src/server.ts` and `packages/voice/src/text-stream.ts` as the external drift/control gate.
3. Keep `stable: false` until the guarded-`any` decision is implemented and the three-repository external gate stays green.
