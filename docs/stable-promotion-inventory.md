...
|    6 | `antidrift/no-trivial-selector-wrapper` | adapter convention documented  | continue                           | Chaski, Codebase Atlas           | Chaski, Codebase Atlas                              | Structural inference-appeasement wrappers are core type-contract scope; Codebase Atlas now supplies a non-name-gated `fullExcerpt(file) { return file.source; }` drift gate. Intentional adapter callbacks use a reasoned local disable until repeated use-site pressure justifies an exemption. |
|    7 | `antidrift/require-effect-deps`         | escape convention documented   | continue                           | Chaski, Claude Code Source       | Chaski, Codebase Atlas, Sudocode, Murderbox         | Exhaustive-deps misses the no-array case and external drift now replicates it; Claude Code Source also proves intentional every-render layout effects are real. The accepted convention is a rule-specific disable with a required reason.                                  |
...
This slice resolved the Zod helper-result and SQL static-fragment blockers with real-program gates. The follow-up selector/effect pass documented escape conventions without adding brittle comment or prop-name heuristics.

1. Collect a real consumer that uses `// eslint-disable-next-line antidrift/require-effect-deps -- ...` for an intentional every-render effect, or explicitly accept the convention as covered by maintained disable-description linting.
2. Collect repeated adapter-callback pressure before considering a use-site-aware exemption for `antidrift/no-trivial-selector-wrapper`.
