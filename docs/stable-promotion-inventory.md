...
|    3 | `antidrift/no-redundant-zod-parse`      | negative-gate needed           | continue                           | Chaski, Murderbox                | Chaski, Codebase Atlas, Murderbox                   | Parse-at-edge/reparse discipline is original scope; external production drift exists, and typed schema-contract assertions are now excluded only in direct throw assertion callbacks. Stable promotion needs a real gate for the consumed-parse edge or an explicit acceptance. |
...
The `antidrift/no-redundant-zod-parse` test assertion scope decision is implemented: schema-contract reparses inside direct throw assertion callbacks are clean, while production service-to-boundary reparses and same-schema value reparses still report. Claude Opus 4.8 advisory review completed on June 4, 2026 and recommended keeping `stable: false` until a real negative gate proves a consumed parse inside a throw assertion still reports, or that edge is explicitly accepted.
...
3. Seek a real consumed-parse-in-assertion drift/control case, or document why that edge is accepted before stable promotion.
