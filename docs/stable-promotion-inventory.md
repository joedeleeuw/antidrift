...
The `antidrift/no-appeasement-cast` copy-backed remediation phase is complete enough for now. Broad inventory, advisory review, measured upstream delta, and four repair patterns are recorded; further work should be real consumer cleanup or the next validation-boundary rule, not more copy-only remediation.

1. Stop copy-only remediation for `no-appeasement-cast` unless a new false-positive concern appears.
...
3. Rerun `pnpm policy:benchmark-unsafe-type-assertion` after any real consumer cleanup to confirm antidrift cast-family findings fall while upstream-only noise does not determine custom-rule status.
