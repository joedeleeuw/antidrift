...
| ---: | --------------------------------------- | ------------------------------ | ---------------------------------- | -------------------------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|    1 | `antidrift/no-appeasement-cast`         | remediation-started            | continue                           | Chaski, Codebase Atlas, Sudocode | Chaski, Codebase Atlas, antidrift | Type-escape hatches are core scope; upstream benchmark proved `@typescript-eslint/no-unsafe-type-assertion` is a much broader superset. Initial production repair patterns are proven in bracket-prefixed Chaski and Codebase Atlas copies; continue cleanup before stable. |
...
Work on `antidrift/no-appeasement-cast` first. Broad inventory, advisory review, measured upstream delta, and initial copy-backed Chaski/Codebase Atlas remediation evidence are complete enough to keep this as the lead promotion candidate.

1. Continue remediation across the remaining production drift buckets, starting with serialized data casts and caught-error normalization.
2. Keep `@typescript-eslint/no-unsafe-type-assertion` as an optional benchmark/strict-policy comparison, not as a default replacement.
3. Rerun `pnpm policy:benchmark-unsafe-type-assertion` after cleanup to confirm antidrift cast-family findings fall while upstream-only noise does not determine custom-rule status.
