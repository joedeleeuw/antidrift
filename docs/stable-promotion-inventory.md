...
This slice resolved the effect escape convention and rechecked the selector and SQL promotion blockers with real-program scans. The effect rule is stable. The SQL rule is stable after classifying the remaining real findings and narrowing serialized payload false positives. The selector rule stays structural because adapter pressure has only one strong real case.

1. Keep `antidrift/no-trivial-selector-wrapper` structural until repeated adapter-callback pressure appears in real repositories.
2. Defer `antidrift/no-async-array-method` unless another production drift replication appears; it is deterministic but currently lower leverage.
