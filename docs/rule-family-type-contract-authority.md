...
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Contract appeasement projection   | `antidrift/no-contract-appeasement-projection`                                                                                                                                                                     | Ready                                   | TypeChecker plus structural return-flow proof                                   | Internal helpers that project parameters, parameter members, destructured bindings, or guarded same literals into explicit return contracts.       | Exported boundaries, public API methods, transformed selectors, parser-backed conversions, and real remappings.                                          |
| Inferred local return ownership   | default-off `antidrift/no-redundant-local-return-type`                                                                                                                                                             | Under-proven inventory                  | TypeChecker property identity plus enclosing contextual return ownership        | Nested shorthand-object implementations whose annotations duplicate inference while every call is constrained by an immediate enclosing contract.  | Inferred owners, unconstrained calls, function-value escapes, direct or indirect public exposure, and broader private-helper annotations.                |
| Type escape casts                 | default-on `antidrift/no-appeasement-cast`; upstream `@typescript-eslint/no-unsafe-type-assertion` for broad/double assertions                                                                                     | Narrow blocking plus ecosystem coverage | TypeChecker plus cast syntax                                                    | `any`/`unknown` paths into named contracts; broad unsafe assertions and double-cast tunnels are delegated to maintained ecosystem coverage.        | Guards, schema parses, brand constructors, and typed SDK conversions whose source is not `any` or `unknown`.                                             |
...
| Broad shape probing               | default-off `antidrift/no-underchecked-type-predicate`, default-off `antidrift/no-defensive-shape-probing`                                                                                                         | Under-proven inventory                  | TypeChecker plus AST/control-flow checks                                        | Predicates or normalizers that probe a token shape while claiming a larger object contract.                                                        | Field-complete guards, schema delegation, primitive predicates, typed union discriminants, and typed maps.                                               |
| Contract shape duplication        | default-on `antidrift/no-structural-type-fork`, default-on `antidrift/no-canonical-model-fork`, default-off `antidrift/no-inline-structural-type-at-use-site`, default-on `antidrift/no-nullable-positional-tuple` | Mixed blocking and inventory            | TypeChecker, registry facts, and deterministic AST                              | Local structural copies of configured owner types, exported inline object contracts, and ambiguous nullable positional tuples.                     | Imported owner types, named DTO/input contracts, schema-inferred types, and named object ranges.                                                         |
...
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Contract appeasement projection   | `export function selectSource(file: ParsedFile): string { return file.source; }`                                             | Exported functions are API boundaries; their return type can be part of the public contract.                                 |
| Contract appeasement projection   | `function parseAction(raw: unknown): MemoryResult["action"] { return ActionSchema.parse(raw); }`                             | Schema parsing constructs the target contract instead of projecting another contract into it.                                |
| Inferred local return ownership   | `function build() { const selectedFor = (): Selection => ({ configs, id, selected }); return selectedFor(); }`               | The enclosing function has no explicit return owner, so removing the nested annotation could remove the only contract check. |
| Inferred local return ownership   | `function createApi(): Api { const build = (): Result => ({ value }); return { build }; } export const api = { createApi };` | The nested function value reaches an indirect public export; its annotation can be part of that exposed callable contract.   |
...
| Validation-boundary provenance    | `const input = InputSchema.parse(JSON.parse(typedJsonString));`                                                              | A string-typed input is parsed and immediately validated at the boundary.                                                    |
...
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| Contract appeasement projection   | `function fullExcerpt(file: ParsedFile): string { return file.source; }`                                                                                                 | The helper only repeats a property access while adding an explicit return contract. Inline the member access or let the helper infer.                                                                    | `antidrift/no-contract-appeasement-projection`                           |
| Contract appeasement projection   | `function actionForEventKind(eventKind: EventKind): MemoryResult["action"] \| undefined { if (eventKind === "recall") return eventKind; }`                               | The helper projects one owned discriminant into another result contract instead of constructing that contract at its owner.                                                                              | `antidrift/no-contract-appeasement-projection`                           |
| Inferred local return ownership   | `function selectedData(): Record<Id, Selection> { const selectedFor = (id: Id): Selection => ({ configs, id, selected }); return { default: selectedFor("default") }; }` | The nested annotation repeats exact shorthand inference, and the enclosing return contract still checks every call result after removal.                                                                 | `antidrift/no-redundant-local-return-type`                               |
