...
| `diff+task+graph` | agent-ops | hook, policy script, inventory signal, or workflow automation |
...
| --- | --- | --- | --- | --- | --- | --- |
| Diff-scope creep | Frustration pass 2026-06-10: "why are you changing so much fucking code dude"; "dude the diff is like 200 lines"; "why do we need" | `diff+task+graph` | agent-ops | Hook or policy-script research | Changed-line/task-scope baseline and clean controls | form-bucket |
...
| Net-new bias as behavior | First harvest 2026-06-10: "We should use `SettingsService` instead since it's already imported"; "we should have helpers for this already" | `diff+task+graph` | agent-ops primary, architecture backstop | Hook or inventory signal | Existing-owner discovery baseline against repo graph | form-bucket |
...
- Diff-scope creep guard — "why do we need 100 more lines of code?", "why are you changing so much fucking code dude" recur across months. Agent-ops hook/policy-script research (changed-lines budget vs task scope), not ESLint.
...
| Diff-scope creep: "why are you changing so much fucking code dude"; "dude the diff is like 200 lines"; "i don't want the god damn churn, so unfucking do it"; "why do we need" (15+ variations) | Many, all three stores | Promoted to a named hook-research row — the highest-recurrence complaint in the archive after error swallowing. |
...
- Module-coupling and over-abstraction judgments ("you don't see how these could be entirely independent", "you are an idiot" simple-href case) — feed the diff-scope hook candidate, not a syntax rule; matches the locked `no-thin-typed-factory-wrapper` lesson.
...
None of the 15 new-candidate groups passes the gates as a new rule row; each folds into an existing row or the hook layer: unnecessary-fallback-path → `no-fallback-to-empty`; multi-pass-parsing → validation-boundary / same-schema-recertification; over-calling-apis → effect-fetch-waterfall research; unjustified-complexity, unnecessary-service-layer, and idiomatic-pattern-not-followed → the diff-scope/net-new hook cluster; unnecessary-fixture-setup → test-integrity guidance. Below-gate singles stay noted in the report only (config-as-code, destructuring-clarity, prevent-instead-of-instrument). All `generalizable=false` items were Chaski-specific tooling.

### The real finding: the hook layer dominates

Roughly 225 of the 545 keepers are agent-behavior complaints, not lintable syntax. Taxonomy by recurrence: diff-scope-creep 60, net-new-bias-as-behavior 25, general agent-behavior 21, incomplete work 18, skip-verification 9, over-automation 6, env-management 6, error-handling protocol 5, scope-drift 5, instruction-skipping 5, plus 15 smaller modes (full list in the report). The archive's verdict: the package's agent-ops/hook surface — not additional ESLint rules — is where the next marginal value lives.
